...
class MobileNetV2Logit(nn.Module):
    def __init__(self, config_file, architecture_config, sample_size=224):
        super(MobileNetV2Logit, self).__init__()
...
def get_model(**kwargs):
    ...
    Returns the model.
    """
    model = MobileNetV2Logit(**kwargs)
    return model


if __name__ == "__main__":
    model = MobileNetV2Logit()
    data = Variable(torch.randn(1, 3, 16, 112, 112))
    output = model(data)
    print(output.size())